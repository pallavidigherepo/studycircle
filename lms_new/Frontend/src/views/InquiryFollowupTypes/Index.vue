    ...
    <div>
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                {{ t("inquiry_followup_types.Inquiry Followup Types") }}
            </h2>
        </div>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="intro-y box p-5">
                    ...
                    <form @submit.prevent="submitForm" class="validate-form">
                        <div>
                            <label for="inquiry_followup_type-name" class="form-label">{{
                                    t("inquiry_followup_types.Name")
                                }}</label>
                            <input
                                id="inquiry_followup_types-name"
                                type="text"
                                class="form-control w-full"
                                :placeholder="t('inquiry_followup_types.Name')"
    ...
    return {
        name: {
            required: helpers.withMessage(
                "Please enter name of inquiry followup type.",
                required
            ),
        },
    };
...
async function submitForm(event) {
    ...
    if (!v$.value.$error) {
        ...
        await store
            .dispatch("inquiry_followup_types/save", model.value)
            .then(() => {
...
function deleteI(item) {
    store.dispatch("inquiry_followup_types/delete", item.id);
}
// END: Delete
</script>
