...
async function submitForm(event) {
  ...
  if (!v$.value.$error) {
    ...
    await store
      .dispatch("fee_types/save", model.value)
      .then(() => {
...
function deleteI(item) {
  store.dispatch("fee_types/delete", item.id);
}
// END: Delete
</script>
<template>
      ...
      <div class="col-span-12 lg:col-span-4">
        <div class="intro-y box p-5">
          ...
          <form @submit.prevent="submitForm" class="validate-form">
            <div>
              <label for="course-type-name" class="form-label">Name</label>
              <FormInput
                id="course-type-name"
                type="text"
