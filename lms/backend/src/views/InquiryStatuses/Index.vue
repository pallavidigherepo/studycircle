    ...
    <div>
        <div class="intro-y flex flex-col sm:flex-row items-center mt-8">
            <h2 class="text-lg font-medium mr-auto">
                {{ t("inquiry_statuses.Inquiry Statuses") }}
            </h2>
        </div>
            ...
            <div class="col-span-12 lg:col-span-4">
                <div class="intro-y box p-5">
                    ...
                    <form @submit.prevent="submitForm" class="validate-form">
                        <div>
                            <label for="inquiry_status-name" class="form-label">{{
                                    t("inquiry_statuses.Name")
                                }}</label>
                            <input
                                id="inquiry_status-name"
                                type="text"
                                class="form-control w-full"
                                :placeholder="t('inquiry_statuses.Name')"
    ...
    return {
        name: {
            required: helpers.withMessage(
                "Please enter name of inquiry status.",
                required
            ),
        },
    };
...
async function submitForm(event) {
    ...
    if (!v$.value.$error) {
        ...
        await store
            .dispatch("inquiry_statuses/save", model.value)
            .then(() => {
...
function deleteI(item) {
    store.dispatch("inquiry_statuses/delete", item.id);
}
// END: Delete
</script>
