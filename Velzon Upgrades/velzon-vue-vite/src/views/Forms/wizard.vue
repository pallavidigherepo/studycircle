...
function toggleTabArrow(tab) {
  activeTabArrow.value = tab
}

function toggleTabVerti(tab) {
  activeTabVerti.value = tab
}
  ...
  document.querySelectorAll(".form-steps").forEach(form => {
    ...
    form.querySelectorAll(".previestab").forEach(prevButton => {
      prevButton.addEventListener("click", () => {
        ...
        const doneTabs = prevButton.closest("form").querySelectorAll(".custom-nav .done")
        const totalDone = doneTabs.length
        if (doneTabs[totalDone - 1]) {
          doneTabs[totalDone - 1].classList.remove('done')
        }
    ...
    tabButtons.forEach((button, i) => {
      ...
      button.addEventListener("click", () => {
        ...
        if (getProgreebar) {
          ...
          const percent = (i / totalLength) * 100
