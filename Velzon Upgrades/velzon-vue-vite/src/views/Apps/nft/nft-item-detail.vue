...
const startCountDownDate = (dateVal) => {
  return new Date(dateVal).getTime();
};

onMounted(() => {
  const setEndDate1 = "March 19, 2024 6:0:0";
  const cdd1 = startCountDownDate(setEndDate1);

  const interval = setInterval(() => {
    const now = new Date().getTime();
    const distance = cdd1 - now;
    ...
    // Prefix single-digit numbers with 0
    days = days < 10 ? "0" + days : days;
    hours = hours < 10 ? "0" + hours : hours;
    minutes = minutes < 10 ? "0" + minutes : minutes;
    seconds = seconds < 10 ? "0" + seconds : seconds;

    const countdownEl = document.querySelector("#auction-time-1");
    ...
    if (countdownEl) {
      if (distance >= 0) {
        ...
        clearInterval(interval); // Stop interval after countdown ends
      }
    }
  }, 1000);
});
