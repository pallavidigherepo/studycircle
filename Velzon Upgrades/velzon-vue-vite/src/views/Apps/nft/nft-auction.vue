...
const startCountDownDate = (dateVal) => new Date(dateVal).getTime();
...
const initializeCountdowns = () => {
  ...
  countdowns.value = endDates.map(startCountDownDate);
};
...
const updateCountdown = (index, countdown) => {
  const interval = setInterval(() => {
    const now = new Date().getTime();
    const distance = countdowns.value[index] - now;
    if (distance < 0) {
      clearInterval(interval);
      countdowns.value[index] = "00 : 00 : 00 : 00";
    } else {
      const { days, hours, minutes, seconds } = formatTime(distance);
      countdowns.value[index] = `${days} : ${hours} : ${minutes} : ${seconds}`;
    }
  }, 1000);
};
...
onMounted(() => {
  ...
  countdowns.value.forEach((_, index) => {
    updateCountdown(index, countdowns.value[index]);
  });
});
</script>
