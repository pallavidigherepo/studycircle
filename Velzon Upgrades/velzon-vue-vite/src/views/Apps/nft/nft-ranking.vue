...
import { rankinglist as initialRankingList } from "@/common/data";
...
const rankinglist = ref([...initialRankingList]); // clone to make it reactive

const direction = ref("asc");
const contactsList = ref([...initialRankingList]); // for sorting if needed

// Computed: paginate ranking list
const displayedPosts = computed(() => {
  const from = (page.value - 1) * perPage.value;
  const to = page.value * perPage.value;
  return rankinglist.value.slice(from, to);
});

// Computed: filter displayedPosts by searchQuery
...
const setPages = () => {
  pages.value = [];
  const numberOfPages = Math.ceil(rankinglist.value.length / perPage.value);
...
const onSort = (column) => {
  ...
  const sorted = [...contactsList.value].sort((a, b) => {
    const res = a[column] < b[column] ? -1 : a[column] > b[column] ? 1 : 0;
    return direction.value === "asc" ? res : -res;
  });
  contactsList.value = sorted;
};

// Lifecycle: run on component creation
onMounted(() => {
  setPages();
});
</script>
