...
const createApiModal = ref(false);
const page = ref(1);
const perPage = ref(8);
const pages = ref([]);
const apikeydata = ref([...apiKeyList]);
...
const displayedPosts = computed(() => paginate(apikeydata.value));
...
const setPages = () => {
  const numberOfPages = Math.ceil(apikeydata.value.length / perPage.value);
  pages.value = Array.from({ length: numberOfPages }, (_, i) => i + 1);
};

watch(apikeydata, setPages);
...
const onSort = (column) => {
  ...
  apikeydata.value.sort((a, b) => {
    const res = a[column] < b[column] ? -1 : a[column] > b[column] ? 1 : 0;
    return direction.value === 'asc' ? res : -res;
  });
};

const handleApikeydetails = (e, item) => {
  createApiModal.value = true;
...
const handleApiKeys = () => {
  let id = apikeydata.value.length + 1;
  const apikeyname = document.getElementById('api-key-name').value;
  const apikey = document.getElementById('api-key').value;
  const modalTitle = document.querySelector('.exampleModalLabel').innerHTML;

  if (modalTitle === "Create API Key") {
    if (id && apikeyname && apikey) {
      const data = {
        id,
        name: apikeyname,
        createby: "Anna Adame",
    ...
    apikeydata.value.forEach(item => {
      if (item.id.toString() === id.toString()) {
        if (modalTitle === "Rename API name") {
          item.name = apikeyname;
        } else {
          item.apikey = apikey;
        }
      }
...
const deleteApikey = (event) => {
  Swal.fire({
    ...
    if (result.value) {
      apikeydata.value = apikeydata.value.filter(item => item.id !== event.id);
...
const changestatus = (item) => {
  item.status = item.status === "Active" ? "Disable" : "Active";
};
</script>
