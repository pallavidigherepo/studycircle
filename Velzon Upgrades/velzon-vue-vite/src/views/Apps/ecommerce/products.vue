...
const sliderCustomzie = ref([1000, 3000]);
const productsData = ref([]);
const pages = ref([]);
const page = ref(1);
const perPage = ref(10);
const value = ref(["Watches", "Headset"]);
const defaultOptions = ref({ animationData: animationData });

// Computed
const displayedPosts = computed(() => paginate(productsData.value));
...
const deleteMultiple = () => {
  ...
  items.forEach((ele) => {
    if (ele.checked === true) {
      const trNode = ele.parentNode.parentNode;
  ...
  if (ids_array.length > 0) {
    if (confirm("Are you sure you want to delete this?")) {
      let cusList = productsData.value;
      ids_array.forEach((id) => {
        cusList = cusList.filter((orders) => orders._id !== id);
      });
      productsData.value = cusList;
      document.getElementById("checkAll").checked = false;
      const itemss = document.getElementsByName("chk_child");
      itemss.forEach((ele) => {
        if (ele.checked === true) {
          ele.checked = false;
...
const onSort = (column) => {
  ...
  const sortedArray = [...productsData.value];
...
const setPages = (data) => {
  const numberOfPages = Math.ceil(data.length / perPage.value);
  pages.value = [];
...
const deletedata = (event) => {
  Swal.fire({
    ...
    if (result.value) {
      productsData.value.splice(productsData.value.indexOf(event), 1);
