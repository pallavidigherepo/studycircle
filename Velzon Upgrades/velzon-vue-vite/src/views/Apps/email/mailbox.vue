...
const emailsdata = ref(emails);
const mailChatDetailElm = ref(false);
const modalShow = ref(false);
const modalShow1 = ref(false);
const removeModal = ref(false);
const tabtype = ref('primary');
const menutab = ref('all');
const labeltype = ref('all');
const value = ref(['support@themesbrand.com']);
...
const deleteMail = () => {
  ...
  document.getElementById("delete-record").addEventListener("click", () => {
    document.querySelectorAll(".message-list li").forEach(element => {
...
const checkBoxAll = () => {
  document.querySelectorAll(".checkbox-wrapper-mail input").forEach(element => {
  ...
  const checkall = document.getElementById('checkall');
  const checkboxes = document.querySelectorAll('.checkbox-wrapper-mail input');
  checkboxes.forEach(element => {
    element.addEventListener('click', (event) => {
      ...
      const actionsDisplay = checkedCount > 0 ? 'block' : 'none';
      document.getElementById("email-topbar-actions").style.display = actionsDisplay;
    });
  });
...
const scrollToBottom = (id) => {
  setTimeout(() => {
    ...
    const offsetHeight = document.getElementsByClassName("chat-conversation-list")[0] ? document.getElementById(id).getElementsByClassName("chat-conversation-list")[0].scrollHeight - window.innerHeight + 750 : 0;
    if (offsetHeight) {
...
const changeUserScroll = () => {
  scrollToBottom("users-chat");
};

const openemail = () => {
  document.body.classList.add("email-detail-show");
  document.querySelectorAll(".message-list li.unread").forEach(element => {
...
const changemenu = (menu) => {
  ...
  if (menu !== 'all' && menu !== 'Inbox') {
    tabtype.value = "primary";
    document.getElementById('mailfilter').style.display = 'none';
  }
  labeltype.value = 'changelable';
  menutab.value = menu;
};

const changelable = (lable) => {
  document.getElementById('primarytab').click();
  menutab.value = 'changelable';
  labeltype.value = lable;
};

const emailDetailChange = () => {
  document.querySelectorAll(".message-list li").forEach(item => {
    item.addEventListener("click", () => {
      const subjectTitle = item.querySelector(".subject-title").innerHTML;
      document.querySelector(".email-subject-title").innerHTML = subjectTitle;

      const emailTitleLeftName = item.querySelector(".title-name").innerHTML;
      document.querySelectorAll(".accordion-item.left").forEach(subitem => {
        subitem.querySelector(".email-user-name").innerHTML = emailTitleLeftName;
        const userImg = item.querySelector(".mail-userimg").value;
        subitem.querySelector("img").setAttribute("src", userImg);
      });

      const messageUserName = document.querySelector(".user-name-text").innerHTML;
      const usermailProfile = document.querySelector(".header-profile-user").getAttribute("src");
      document.querySelectorAll(".accordion-item.right").forEach(subitem => {
...
onMounted(() => {
  ...
  document.querySelectorAll(".email-chat-list a").forEach(item => {
    item.addEventListener("click", () => {
      document.getElementById("emailchat-detailElem").style.display = "block";
      mailChatDetailElm.value = true;

      const chatUserList = document.querySelector(".email-chat-list a.active");
      if (chatUserList) chatUserList.classList.remove("active");
      item.classList.add("active");

      const username = item.querySelector(".chatlist-user-name").innerHTML;
      const userProfile = item.querySelector(".chatlist-user-image img").getAttribute("src");

      document.querySelector(".email-chat-detail .profile-username").innerHTML = username;
      const conversationImg = document.getElementById("users-conversation");
      Array.from(conversationImg.querySelectorAll(".left .chat-avatar")).forEach(subitem => {
        subitem.querySelector("img").setAttribute("src", userProfile || "assets/images/users/user-dummy-img.jpg");
      });
    });
  });

  document.getElementById("emailchat-btn-close").addEventListener("click", () => {
    document.getElementById("emailchat-detailElem").style.display = "none";
    mailChatDetailElm.value = false;
    document.querySelector(".email-chat-list a.active").classList.remove("active");
  });

  document.querySelectorAll('.tabtype a').forEach(mailTab => {
    mailTab.addEventListener("click", () => {
      const chatUserList = document.querySelector(".tabtype a.active");
      if (chatUserList) chatUserList.classList.remove("active");
      mailTab.classList.add('active');
    });
  });

  document.querySelectorAll('.mail-list a').forEach(mailTab => {
    mailTab.addEventListener("click", () => {
      const chatUserList = document.querySelector(".mail-list a.active");
      if (chatUserList) chatUserList.classList.remove("active");
      mailTab.classList.add('active');
    });
  });
  ...
  document.querySelectorAll(".email-menu-btn").forEach(item => {
  ...
  document.querySelector('.email-wrapper').addEventListener('click', () => {
                    ...
                    <div class="message-list-content mx-n4 px-4 message-list-scroll" id="emailsdatas" data-simplebar>
                        ...
                        <ul class="message-list" id="mail-list-data" style="display: none">
                            ...
                            <li class="bg-transparent text-center">
                                <BButton type="button" pill variant="soft-info" size="sm" class="btn-rounded px-2"><i
