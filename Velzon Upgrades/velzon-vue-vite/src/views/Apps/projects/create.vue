...
const value = ref(["C#", "HTML", "CSS"]);
const value1 = ref(["Inprogress"]);
const value2 = ref(["High"]);
const value3 = ref(["Private"]);
const value4 = ref(["Designing"]);
const value5 = ref(["Ellen Smith"]);
...
const editorData = ref(`
  ...
  are members of the same family. Their separate existence is a myth. For science, 
...
// Watch uploaded files (optional side-effect)
watch(() => [...files.value], (newVal) => {
  // Add any custom behavior here
});

// Delete attachment from DOM (imperative approach)
const deleteRecord = (ele) => {
  ele.target?.closest(".file-entry")?.remove(); // safer DOM traversal
};

// Setup Vuelidate (schema can be added as needed)
