...
// Job Summary Chart Data
const jobsummarychartseries = ref([
  { name: 'Application Sent', data: [33, 28, 30, 35, 40, 55, 70, 110, 150, 180, 210, 250] },
  { name: 'Interviews', data: [20, 26, 45, 32, 42, 53, 59, 70, 78, 97, 110, 125] },
  { name: 'Hired', data: [12, 17, 45, 42, 24, 35, 42, 75, 102, 108, 156, 199] },
  { name: 'Rejected', data: [8, 13, 22, 27, 32, 34, 46, 59, 65, 97, 100, 110] }
]);

// Job Summary Chart Options
const jobsummarychartchartOptions = ref({
  ...
  fill: { opacity: 0.06, colors: getChartColorsArray('["--vz-success", "--vz-primary", "--vz-info", "--vz-danger"]'), type: "solid" }
});

// Your Network Chart Data
const yournetworkchartseries = ref([
  { name: 'Following', data: [80, 50, 30, 40, 100, 20] },
  { name: 'Followers', data: [20, 30, 40, 80, 20, 80] }
]);

// Your Network Chart Options
const yournetworkchartchartOptions = ref({
  chart: { height: 341, type: "radar", dropShadow: { enabled: true, blur: 1, left: 1, top: 1 }, toolbar: { show: false } },
  stroke: { width: 2 },
  fill: { opacity: 0.2 },
  legend: {
    show: true,
...
// User by Device Data
const userbydeviceseries = ref([78.56, 105.02, 42.89]);

// User by Device Chart Options
const userbydevicechartOptions = ref({
  ...
  stroke: { width: 0 },
  yaxis: {
    labels: { formatter: (value) => `${value}k Users` },
    tickAmount: 4,
    min: 0
  },
...
// Visitor Graph Chart Options
const visitorgraphchartOptions = ref({
  legend: { show: false },
  chart: { height: 350, type: "treemap", toolbar: { show: false } },
  title: { text: "Visitors Location", align: "center", style: { fontWeight: 500 } },
  colors: getChartColorsArray('["--vz-primary", "--vz-secondary", "--vz-success", "--vz-info", "--vz-warning", "--vz-danger"]'),
  plotOptions: { treemap: { distributed: true, enableShades: false } }
});

// Small Area Chart Colors and Options
const chartcolors = ref({ colors: ["#f06548"] });
const chartoptions = ref({
  chart: { width: 140, type: "area", sparkline: { enabled: true }, toolbar: { show: false } },
  dataLabels: { enabled: false },
