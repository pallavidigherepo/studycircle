...
const perPage = ref(3);
const pages = ref([]);
const joblistData = ref(joblist);

const series = ref([98, 63, 35]);
...
// Computed properties
const displayedPosts = computed(() => paginate(joblistData.value));
...
// Methods
const setPages = () => {
  let numberOfPages = Math.ceil(joblistData.value.length / perPage.value);
  pages.value = [];
...
const handleSubmit = () => {
  ...
  if (submitted.value && (event.value.name && event.value.designation && event.value.experience && event.value.location && event.value.type && event.value.tags && event.value.salary && event.value.description)) {
    const data = {
    ...
    joblistData.value.unshift(data);
    createJobModal.value = false;
  }
};

const toggleModal = () => {
  createJobModal.value = true;
  event.value = {};
