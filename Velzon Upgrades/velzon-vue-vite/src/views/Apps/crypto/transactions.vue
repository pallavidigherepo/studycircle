...
const defaultOptions3 = { animationData: animationData3 };
const defaultOptions4 = { animationData: animationData4 };

// Transactions data
const transactionsData = ref([...transactions]);

// Pagination setup
const setPages = () => {
  const numberOfPages = Math.ceil(transactionsData.value.length / perPage.value);
  pages.value = Array.from({ length: numberOfPages }, (_, index) => index + 1);
};
...
const displayedPosts = computed(() => {
  return paginate(transactionsData.value);
});
...
const onSort = (column) => {
  ...
  const sortedArray = [...transactionsData.value];
  sortedArray.sort((a, b) => {
    const res = a[column] < b[column] ? -1 : a[column] > b[column] ? 1 : 0;
    return direction.value === "asc" ? res : -res;
  });
  transactionsData.value = sortedArray;
};

// Watch for transactions data changes
watch(transactionsData, setPages);

// Initialize pages on mounted
onMounted(() => {
  setPages();
});
</script>
